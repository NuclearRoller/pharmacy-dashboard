...
export default function App() {
  ...
  // --- Set default latest month for pie chart
  useEffect(() => {
  ...
  // --- Commentary (animated, independent)
  const commentary = useMemo(() => {
    ...
    const overallChange = branchNames.reduce(
      (sum,b) => sum + (latest[b] - (prev[b]||0)),0
    );
  ...
  return (
    <div className="min-h-screen p-6 space-y-6 bg-gray-100">
      <h1 className="text-3xl font-bold text-blue-600">لوحة تحكم الصيدلية</h1>
      <LiveCSV csvUrl={csvUrl} onDataLoaded={setSalesData} />

      {/* ===== COMMENTARY CARD ===== */}
      {commentary.length > 0 && (
        <div className="commentary-card">
          <h2 className="text-xl font-semibold mb-2">ملخص الأداء</h2>
          <div className="space-y-1">
            {commentary.map((line, idx) => (
              ...
              >
                {line.text}
                {line.branch && (
                  <span className="branch-name" style={{ color: branchColors[line.branch] }}>
                    {branchNamesArabic[line.branch]}
                  </span>
                )}
                {line.number != null && (
                  <span className="commentary-number"> {line.number}</span>
                )}
              </p>
            ))}
          </div>
      ...
      {monthlyAverages.length > 0 && (
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xl font-semibold">المتوسطات الشهرية</h2>
      ...
      <div className="bg-white rounded-2xl shadow p-4 space-y-2">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <h2 className="text-xl font-semibold">جميع الفروع</h2>
      ...
      {/* ===== BRANCH LINE CHARTS ===== */}
      <div className="bg-white rounded-2xl shadow p-4">
        <h2 className="text-xl font-semibold mb-3">مخططات الفروع</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {branchNames.map(branch => (
            <BranchLineChart key={branch} branch={branch} title={branchNamesArabic[branch]} color={branchColors[branch]} isMonthlyAverage={lineMode==="averages"} monthlyData={monthlyAverages} dailyData={dailyTotalsSlice}/>
          ))}
        </div>
      </div>
      ...
      {/* ===== TOTAL DAILY SALES ===== */}
      <div className="bg-white rounded-2xl shadow p-4">
        <h2 className="text-xl font-semibold mb-2">إجمالي المبيعات اليومية</h2>
